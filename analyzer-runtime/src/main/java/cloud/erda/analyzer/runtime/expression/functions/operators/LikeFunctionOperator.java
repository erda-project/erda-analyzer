...
public class LikeFunctionOperator implements FunctionOperator {

    @Override
    public String operator() {
        return FunctionOperatorDefine.like;
    }

    @Override
    public boolean invoke(AggregateResult aggregateResult) throws Exception {
        Object value = aggregateResult.getFunction().getValue();
        if (value == null || aggregateResult.getValue() == null) {
            return false;
        }
        return value.toString().contains(aggregateResult.getValue().toString());
    }
}
