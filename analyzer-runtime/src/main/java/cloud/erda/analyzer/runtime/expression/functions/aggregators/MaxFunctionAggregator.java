...
public class MaxFunctionAggregator implements FunctionAggregator {

    private Double max  = null;

    @Override
    public String aggregator() {
        return FunctionAggregatorDefine.MAX;
    }
    ...
    public void merge(FunctionAggregator other) {
        if (other instanceof MaxFunctionAggregator) {
            if (((MaxFunctionAggregator) other).max > max) {
                this.max = ((MaxFunctionAggregator) other).max;
            }
        }
    }
}
